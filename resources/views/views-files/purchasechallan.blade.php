...
<div id="pdf">
	<div style="border: 3px solid;margin: 50px; background-color: white;" >
		...
		<div style="border-bottom: 2px solid;width: 100%">
			<p style="float: left;width: 33.33%;margin-bottom: 0rem;text-align: left;">{{ $purchaseChallan->invoicetype }}</p>
		...
		<div style="display: flex;">
			<div style="width: 65%;border-right: 2px solid;">
				<p>M/S : {{ $purchaseChallan->account_name }}</p>
				<p>{{ $purchaseChallan->city }} - {{ $purchaseChallan->pincode }}</p>
				<p>GST NO : {{ $purchaseChallan->gstno }}</p>
				<p style="clear: both;margin-bottom: 0rem;"></p>
			</div>
		...
		<div style="border-top: 2px solid;background-color: #f2f2f2;display: flex;width: 100%;">
			...
			<p style="width: 6%;">{{ $purchaseChallan->totaldiscount }}</p>
			<p style="width: 11%;">{{ $taxableval }}</p>
			<p style="width: 6.5%;">{{ number_format(str_replace(',','',$purchaseChallan->totaltax) / 2,2) }}</p>
			<p style="width: 6.5%;">{{ number_format(str_replace(',','',$purchaseChallan->totaltax) / 2,2) }}</p>
			<p style="width: 10%;">{{ $purchaseChallan->total }}</p>
		</div>
		<div style="display: flex;">
		...
		<div style="display: flex;">
			<div style="width: 73%;border-top: 2px solid;">
				<p>Total GST : {{ $purchaseChallan->totaltax }}</p>
				<p>Bill Amount : {{ $purchaseChallan->total }}</p>
			</div>
			<div style="width: 27%;border-top: 2px solid;border-left: 2px solid;">
				<div style="display: flex;background-color: #f2f2f2;height: 30px;padding-bottom: 30px;border: 1px solid;" >
					<p style="text-align: left;width: 50%;">Grand Total</p><p style="text-align:left;width: 50%;">{{ $purchaseChallan->total }}</p>
				</div>
				<div>
					Note : {{ $purchaseChallan->notes }}
				</div>
			</div>
		</div>
		<div style="display: flex;">
