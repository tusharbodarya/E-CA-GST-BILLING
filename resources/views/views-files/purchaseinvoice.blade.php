...
<div id="pdf">
	<div style="border: 3px solid;margin: 50px; background-color: white;" >
		...
		<div style="border-bottom: 2px solid;width: 100%">
			<p style="float: left;width: 33.33%;margin-bottom: 0rem;text-align: left;">{{ $purchaseInvoice->invoicetype }}</p>
		...
		<div style="display: flex;">
			<div style="width: 65%;border-right: 2px solid;">
				<p>M/S : {{ $purchaseInvoice->account_name }}</p>
				<p>{{ $purchaseInvoice->city }} - {{ $purchaseInvoice->pincode }}</p>
				<p>GST NO : {{ $purchaseInvoice->gstno }}</p>
				<p style="clear: both;margin-bottom: 0rem;"></p>
			</div>
			<div style="width: 35%;border-bottom: 2px solid;background-color: #f2f2f2;margin-bottom: 40px;">
				<p style="margin-bottom: 0rem;">Bill No : <b>{{ $purchaseInvoice->orderid }}</b></p>
				<p style="margin-bottom: 0rem;">Challen Date : {{ date_format(date_create($purchaseInvoice->created_at),"d/m/Y") }} </p>
				<p style="margin-bottom: 0rem;">Challen No : <b>{{ $purchaseInvoice->challannum }}</b></p>
				<p style="clear: both;margin-bottom: 0rem;"></p>
			</div>
		</div>
		...
		<div style="border-top: 2px solid;background-color: #f2f2f2;display: flex;width: 100%;">
			...
			<p style="width: 6%;">{{ $purchaseInvoice->totaldiscount }}</p>
			<p style="width: 11%;">{{ $taxableval }}</p>
			<p style="width: 6.5%;">{{ number_format(str_replace(',','',$purchaseInvoice->totaltax) / 2,2) }}</p>
			<p style="width: 6.5%;">{{ number_format(str_replace(',','',$purchaseInvoice->totaltax) / 2,2) }}</p>
			<p style="width: 10%;">{{ $purchaseInvoice->total }}</p>
		</div>
		<div style="display: flex;">
		...
		<div style="display: flex;">
			<div style="width: 73%;border-top: 2px solid;">
				<p>Total GST : {{ $purchaseInvoice->totaltax }}</p>
				<p>Bill Amount : {{ $purchaseInvoice->total }}</p>
			</div>
			<div style="width: 27%;border-top: 2px solid;border-left: 2px solid;">
				<div style="display: flex;background-color: #f2f2f2;height: 30px;padding-bottom: 30px;border: 1px solid;" >
					<p style="text-align: left;width: 50%;">Grand Total</p><p style="text-align:left;width: 50%;">{{ $purchaseInvoice->total }}</p>
				</div>
				<div>
					Note : {{ $purchaseInvoice->notes }}
				</div>
			</div>
		</div>
		<div style="display: flex;">
